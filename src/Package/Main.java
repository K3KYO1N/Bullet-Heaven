...
public class Main extends Game2 {
	...
	private static final int PLAYER_BULLET_COOLDOWN = 20; //20

	// powerups
	private Powerup up;
	private boolean powerupExists = false;
	private int powerupCoolDownTick = 0;
	private static final int PU_COOLDOWN = 1000;
	
	private int powerupTick =1000;
	private static final int PU_LIMIT = 280;
	...
	private void circle(int size, double x, double y, double vx, double vy, double distance, double period,
		...
		for(int i=0;i<8;i++) {
			EnemyBullet temp = new EnemyBullet(x, y, distance, i * period / 8, direction, size, size, str);
			add(temp);
	...
	private void removeBullets() {
		...
		if(powerupExists == true) remove(up); //remove powerups
	...
	private void displayMenu() {
		powerupTick =1000;// reset powerup
		removeBullets();
		displayingMenu = true;
		menu.setBounds(getFieldWidth() / 2 - 150, getFieldHeight() / 2 - 200, 300, 200);// setting size
		menu.setHorizontalAlignment(SwingConstants.CENTER);
		if (gameWin == "Win") { 
			menu.setForeground(Color.BLUE);// sets text color to blue for win screen
			menu2.setForeground(Color.BLUE); // sets text color to blue for win screen
		}
		else {
			menu.setForeground(Color.RED);// sets text color to red for lose screen
			menu2.setForeground(Color.RED);// sets text color to red for lose screen
		}
	...
	public void act() {
		...
		if (gameStart == true) {
			...
			if (NKeyPressed() && playerBulletTimeCounter >= PLAYER_BULLET_COOLDOWN) {
				...
				if(powerupTick< PU_LIMIT){
					Bullet temp1 = new Bullet(ship.getX() + 10, ship.getY() - 20);
					temp1.setPowerupSprite();
					add(temp1);
					playerBullets.add(temp1);
					Bullet temp3 = new Bullet(ship.getX() - 15, ship.getY() - 20);
					temp3.setPowerupSprite();
					add(temp3);
					playerBullets.add(temp3);
					Bullet temp4 = new Bullet(ship.getX() + 35, ship.getY() - 20);
					temp4.setPowerupSprite();
					add(temp4);
					playerBullets.add(temp4);
					
					temp.setPowerupSprite();
					temp2.setPowerupSprite();
				}	
				add(temp);
			...
			// spawning powerups
			if (powerupExists == false && powerupCoolDownTick > PU_COOLDOWN) {
				up = new Powerup(getFieldWidth() / 2 - 20, getFieldHeight() / 2);
				add(up);
				powerupCoolDownTick=0;
				powerupExists = true;
			}
			// powerup collision
			if (powerupExists == true && ship.collides(up)){
				remove(up);
				powerupTick = 0;
				powerupExists = false;
